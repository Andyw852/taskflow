...
INCAR 改写（step3 PBE 预收敛 -> step4 HSE 能带）:
    ...
    改/增: SYSTEM, ISTART=1, ISYM=3(HSE 默认，必须显式写，否则会继承 step3 的值),
           ...
           PRECFOCK=Fast, LORBIT=11（轨道投影，fat band 必需）,
...
INCAR_SET = {
    "SYSTEM":   "HSE06 band (step4): read step3 WAVECAR, KPOINTS_OPT path",
    ...
    "LORBIT":   "11",         # ★ 轨道投影：写 PROCAR/PROCAR_OPT（lm 分解权重）→ fat band 必需
...
def override_submit_slurm(submit_path, override):
    """渲染后覆盖 submit.sh 的 nodes/ntasks_per_node/qos（None 项跳过）。返回改动说明。"""
...
def write_kpoints_opt(out_dir, header, coord_subset, tag):
    hdr0 = (header[0].strip() or "k-path for band structure") + tag
...
def build_step4_dir(out_dir, args, ctx, part=None):
    ...
    _sub_changed = override_submit_slurm(os.path.join(out_dir, "submit.sh"), _override)
    if _sub_changed:
    ...
    incar_set["SYSTEM"] = "%s %s%s band (step4; geometry=%s)%s" % (
...
def main():
    ...
    print("输出文件      : LWAVE=.TRUE.（存 WAVECAR）  LORBIT=11（写 PROCAR/PROCAR_OPT，fat band）"
          "  LCHARG=.TRUE.（存 CHGCAR）")
