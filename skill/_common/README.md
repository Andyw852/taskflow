...
skill/
  ...
  band/  elastic/  ke/  kl/ ← 各技能只留自己独有的东西
```
...
`relax_common` 是从 `skill/band/gen_step1_PBE_opt.py` 抽出来的，抽的过程
...
原来两套并存：band 是 tf 层面的 a/b/c 三个步骤，elastic/ke 是一个作业里
`INCAR.s1/s2/s3` + `run_relax.sh` 顺序跑。现在统一由 `STAGE_MODE` 选择，
缺省 `in_job`：

| | `in_job`（缺省） | `tf_stages`（旧 band） |
|---|---|---|
| 排队 | 1 次 | 3 次 |
...
沿用 elastic 那版的看门狗（OUTCAR 停滞 `STALL_MIN` 分钟判卡死）、每段
...
* `tf_stages` 模式：与原 `skill/band/gen_step1_PBE_opt.py` 在同一 2D 结构上
...
* elastic 薄壳（`CELL_POLICY="standard"`）在装了 pymatgen 的环境里跑通，
...
0D 分支）都在池子里，改一次全技能生效。壳薄到什么程度：band 那个从
1083 行变成 22 行。
...
`STAGE_MODE` 四个都是 `in_job`。elastic 原来"3D 恒为单段"，迁移后 3D 也分段；
...
| | 0D 能不能跑 | 说明 |
|---|---|---|
| band step1 | ✅ | `MOL_BRANCH=True` → mol_common，需要 `incar_0d.tpl` / `submit_std_0d.tpl` |
| band step2 | ⚠️ 要补 | `dim_common` 池子版已放行 `DIM=0D`，但还缺 `incar_0d`/`submit_std_0d` 的 step2 版；KPOINTS 要走 `--no-vaspkit` 复用 step1 的 Γ |
| band step3/step4 | ❌ 不该跑 | 高对称路径定义在晶体倒空间，分子没有能带 |
| elastic / ke / kl | ❌ 不该跑 | 弹性张量、晶格热导、形变势对孤立分子都没有定义 |
...
3. 逐技能删副本：`rm skill/band/dim_common.py skill/band/check_common.py
   skill/band/stepconf.py`（`gen_need` 不动）→ 跑一次 `tf ... gen` 确认远端
   拿到的文件 md5 与池子一致 → 再删下一个技能的。
4. `skill/band/gen_step1_PBE_opt.py` 换成薄壳，`gen_need` 里加 `relax_common.py`
...
要注意的两点：elastic 原来只对 2D 分段、3D 恒为单段 ISIF=3，迁移后 3D 也会分段
（想保持旧行为就传 `STAGE_MODE="single"`）；另外 elastic 的 step1 还有
"真空轴自动轮换到 c"（`rotate_vacuum_to_c`）这一步，公共池目前没有，
迁移时要么一并搬进 `relax_common`，要么先留在 elastic 的薄壳里做前置处理。
