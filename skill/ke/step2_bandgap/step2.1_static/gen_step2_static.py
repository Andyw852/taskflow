...
def override_submit_slurm(submit_path, override):
    """渲染后覆盖 submit.sh 的 nodes/ntasks_per_node/qos（None 项跳过）。"""
    ...
    if changed:
        print("[..] submit.sh override Slurm: %s" % ", ".join(changed))
    else:
        print("[..] submit.sh Slurm params: unchanged (SUBMIT_OVERRIDE all None)")
