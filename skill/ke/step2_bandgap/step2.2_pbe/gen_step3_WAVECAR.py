...
INCAR_SET = {
    "SYSTEM": "PBE pre-converge WAVECAR for HSE band (step3)",
    "ALGO":   "Normal",
    "NELM":   "200",
...
def main():
    ...
    if USE_KPOINTS_OPT:
        ...
        with open(kpoints_file, "w") as f:
            f.write("HSE band: %d uniform(weighted) + %d path + %d extra (zero-weight) [%s]\n"
    ...
    _sub_changed = override_submit_slurm(submit_out, SUBMIT_OVERRIDE)
    if _sub_changed:
