...
def estimate_n_struct(nfree_by_order, n_atoms_sc, oversample, n_min=4):
    """各阶 N = ceil(nfree/(3*N_sc))*oversample 取下限；N_STRUCT = 各阶之和。"""
    dof = 3 * n_atoms_sc
    per = {o: max(n_min, math.ceil(nf / dof) * oversample) for o, nf in nfree_by_order.items()}
    n_struct = sum(per.values())
    logging.info("各阶自由参数=%s, DOF=%d -> 各阶结构数=%s, N_STRUCT=%d",
                 nfree_by_order, dof, per, n_struct)
    return n_struct
...
def detect_is_metal(vasprun_path, gap_thr: float = 0.01):
    ...
    logging.info("[金属判据] band gap = %.4f eV → %s（阈值 %.3f eV）",
...
def compute_band_and_check(phonon, band_dir, C, nac_vasprun=None):
    ...
    if bsobj is None:
        raise RuntimeError("phonon.band_structure 仍为 None：_resolve_band_path 未成功运行 band")
    ...
    # ── 声子谱绘图数据（band.yaml，phonopy 标准 yaml，可复现色散谱）───────────
...
# phonon_band_data  str      声子谱绘图数据 yaml 路径（band.yaml）
    ...
    try:
        ...
        if DO_NAC and C["skip_nac_if_metal"] and DO_RELAX:
            ...
            if is_metal:
                ...
                metal_nac_note = (f"检测到金属（band gap ≈ {gap:.3f} eV < "
        ...
        if DO_NAC and C["skip_nac_if_metal"] and not DO_RELAX:
            ...
            if is_metal:
                ...
                metal_nac_note = (f"检测到金属（band gap ≈ {gap:.3f} eV），已跳过 NAC。"
        ...
        if METHOD == "random":
            ...
            from phonopy.interface.vasp import write_vasp
            unitcell_path = SUBDIRS["band"] / "POSCAR-unitcell"
            ...
            disp_yaml = SUBDIRS["band"] / "phonopy_disp.yaml"
            ...
            disp_yaml = SUBDIRS["band"] / "phono3py_disp.yaml"
        ...
        fc2_disp = SUBDIRS["band"] / "fc2.hdf5"
        if fc2_disp.is_file():
            result["force_constants_fc2"] = str(fc2_disp)
        for cand in (SUBDIRS["band"] / "fc3.hdf5",
...
def fit_fc2_hiphive(prim, supercell_atoms, SUPERCELL, dft_dir, SUBDIRS, C, nac_vasprun):
    ...
    fc2_path = SUBDIRS["band"] / "fc2.hdf5"
    ...
    if DO_KAPPA:
        ...
        fcs.write_to_phono3py(str(SUBDIRS["band"] / "fc3.hdf5"))   # 展示用，与 fc2 同目录
    ...
    result = compute_band_and_check(phonon, SUBDIRS["band"], C, nac_vasprun)
    return result, fcs
...
def fit_fc2_findiff(SUPERCELL, dft_dir, SUBDIRS, used_indices, C, nac_vasprun):
    ...
    return compute_band_and_check(phonon, SUBDIRS["band"], C, nac_vasprun)
...
def fit_fc2fc3_findiff(prim, SUPERCELL, dft_dir, SUBDIRS, C, nac_vasprun):
    ...
    fc2_path, fc3_path = SUBDIRS["band"] / "fc2.hdf5", SUBDIRS["band"] / "fc3.hdf5"
    ...
    logging.info("fc2.hdf5 + fc3.hdf5 已导出: %s", SUBDIRS["band"])
