...
对 v0.1（简化有限差分四步）的彻底重构。对齐你 band/elastic/ke 的成熟架构：
...
|---|---|---|---|---|
| S1 | step1_std_opt | 结构优化（复用 band/elastic/ke 同一脚本，一字不差） | vasp_std | OUTCAR 收敛 |
...
## 安装
把整个 `kl/` 放到 `~/software/taskflow/skill/kl/`，然后：
```
tf skills            # 应看到 kl 0.2 / 6 步 / 启用
```

## 用法
```
tf -tt kl -p <材料> init      # 生成 材料/kl/project_setting/（含 templates/<步骤>/）
tf -tt kl -p <材料>           # 看步骤链
# 按序推进各步（与你 band/elastic 一致的 advance/run 流程）
```

## 改参数（不要改脚本，改 step.conf）
```
tf -tt kl -p <材料> -j <步骤> conf                    # 看某步最终生效值 + 来源
tf -tt kl -p <材料> -j step4_disp conf --set params.METHOD=alm
tf -tt kl -p <材料> -j step4_disp conf --set params.SUPERCELL="3 3 3"
tf -tt kl -p <材料> -j step6_kappa conf --set params.KAPPA_MESH="24 24 24"
```
